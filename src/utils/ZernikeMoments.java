...
public class ZernikeMoments
{
	 ...
	 * zer_mom(double[] xvals, double[] yvals, int npoints, double ww, double hh,
	 * double cx, double cy, int n, int m)
	 */
	public static Complex zer_mom(double[] xvals,
																double[] yvals,
																int npoints,
																int n,
																int m)
	{
		...
		{
			throw new IllegalArgumentException("zer_mom: n=" + n
																					+ ", m="
																					+ m
																					+ ", n-|m|="
																					+ diff);
	 ...
	 * the height of the bounding box of the shape. (cx, cy) is the centroid point
	 * of the shape. n and m are the moment order. 'zer_mom' computes A(n,m).
	 */
	public static Complex zer_mom(double[] xvals,
																double[] yvals,
																int npoints,
																...
																int m)
	{
		...
		radius = hh / 2;
		j_scale = Math.sqrt(2) * radius; // note we want to construct a circle to
		...
		for (int i = 0; i < npoints; i++)
		{
			...
			if (((x * x + y * y) <= 1.0))
			{// we ignore (x,y) not in the unit circle
				v = zer_pol(n, m, x, y);
	 ...
	 * are returned in the order of (0,0)(1,1)(2,0)(2,2)(3,1)(3,3) and so on,
	 * where m<=n and n = 0...order.
	 *
	 * xvals and yvals are the coordinates of the shape. npoints indicates the
	 * total number of points in the digitized shape. ww is the width and hh is
	 * the height of the bounding box of the shape. (cx, cy) is the centroid point
	 * of the shape. n and m are the moment order. 'zer_mom' computes A(n,m).
	 */
	public static Complex[] zer_mmts(	int order,
																		double[] xvals,
																		double[] yvals,
																		int npoints,
																		double ww,
